<nav x-data="{ open: false }" class="bg-gray-900 border-b border-gray-800">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-gray-300 hover:text-white text-sm font-medium border-b-2 border-transparent hover:border-gray-300 pb-1" activeClass="text-white border-white">
                        {{ __('Dashboard') }}
                    </x-nav-link>

                    <x-nav-link :href="route('about')" :active="request()->routeIs('about')" class="text-gray-300 hover:text-white text-sm font-medium border-b-2 border-transparent hover:border-gray-300 pb-1" activeClass="text-white border-white">
                        {{ __('About') }}
                    </x-nav-link>
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-400 hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center gap-1">
                                ...
                                <span class="text-xs text-blue-400">({{ Auth::user()->role }})</span>
                            </div>
                            <div class="ms-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-800 border-t border-gray-700">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('about')" :active="request()->routeIs('about')" class="text-gray-300 hover:text-white hover:bg-gray-700">
                {{ __('About') }}
            </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-gray-700">
            <div class="px-4">
                <div class="font-medium text-base text-gray-200">
                    {{ Auth::user()->name }} <span class="text-sm text-blue-400">({{ Auth::user()->role }})</span>
                </div>
